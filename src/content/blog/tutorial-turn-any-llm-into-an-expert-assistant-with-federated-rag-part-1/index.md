---
title: "Tutorial: Turn Any LLM into an Expert Assistant with Federated RAG – Part 1"
slug: tutorial-turn-any-llm-into-an-expert-assistant-with-federated-rag-part-1
titleMax: 42
date: 2025-10-24T21:21:32
updated: 2025-10-28T13:46:24
categories: [product, research]
tags: [federated-learning, tutorials, privacy-preserving-machine-learning, large-language-models-llms, privacy-enhancing-technologies-pets, syftbox]
authors: [siddhant-rai, irina-bejan]
cover: ./cover.jpg
coverAlt: ""
hideCover: true
cardText: dark
...
<!-- TODO(content): HubSpot form(s) auto-embedded as <FormEmbed> — verify placement; toc_offset_top_px=240 (TOC start offset — maps to TocAnchor) -->
...
_Sign up here to get new learning resources sent to your inbox as they are published:_

<div class="hs-form-embed" data-form-id="77c443cd-2915-4c89-9861-f32ff767e46e" data-portal-id="6487402" data-region="na1"></div>
...
<figure class="wp-block-image size-full is-style-full-width-style">

![](./media/openmined-live-graphics-2.svg)
...
Imagine a hospital, a law office, or just a friend with private notes. Each keeps their own _local_ index.
...
![](./media/openmined-live-graphics-5.svg)

</figure>

_This runs on each peer’s machine in a distributed network (Distributed Indexing), so that the index and documents remain private to each peer. Nothing is shared._
...
This is a local retrieval step, so each each node only searches _its own private index_. No raw data leaves the node until we run a federated query (next step).
...
![](./media/openmined-live-graphics-6.svg)

</figure>

Then if someone submits a question (e.g., _“Who in my friends group knows LLM agents?”_); we would like to use our FedRAG to respond.
...
So far, we’ve only simulated nodes on _our laptop_. Now, let’s connect to a **real network** where illustrative peers already exist (e.g. Hacker News, arXiv, GitHub).
...
We need one LLM model that can take insights from all peers and stitch them into a final answer – here, we are using Claude. There’s more models linked to the network that you can try _– note that when using the network for the first time, you get $20 worth of credits to explore freely anything._
...
Once a query is run, each source computes on their data the local retrieval operation _(just like in Section 2!)_ and returns top matches. The pipeline, acting as the `FederatedCoordinator`, will concatenate the sources into the model context and then call the model (the LLM we chose).
...
The demo gives a glimpse of what’s ahead: AI that learns _from_ data without ever having to _own_ it.  
...
<figure class="wp-block-image size-full is-style-wide-style">

![](./media/openmined-live-graphics-9.svg)
...
_If you’re curious how **Federated RAG** can tackle real-world challenges like privacy, trust, scaling, data monetisation or integrating it into your apps and workflows, make sure to sign up for the series_:

<div class="hs-form-embed" data-form-id="77c443cd-2915-4c89-9861-f32ff767e46e" data-portal-id="6487402" data-region="na1"></div>

</div>

<p class="has-text-align-center">_Till then happy Learning. Bye 👋_</p>
...
-   [Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks](https://arxiv.org/abs/2005.11401) (Lewis et al., 2020) — _The original RAG paper from Facebook AI_
